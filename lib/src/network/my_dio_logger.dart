...
class MyDioLogger extends Interceptor {
  ...
  /// InitialTab count to devLog json response
  ...
  void devLog(Object ob) => dev.log(ob.toString(), name: 'DevLog');
  ...
  void _printPrettyMap(
    ...
    if (isRoot || isListItem) devLog('║$initialIndent{');
    ...
    data.keys.toList().asMap().forEach((index, dynamic key) {
      ...
      if (value is Map) {
        if (compact && _canFlattenMap(value)) {
          devLog('║${_indent(_tabs)} $key: $value${!isLast ? ',' : ''}');
        } else {
          devLog('║${_indent(_tabs)} $key: {');
          _printPrettyMap(value, tabs: _tabs);
        }
      } else if (value is List) {
        if (compact && _canFlattenList(value)) {
          devLog('║${_indent(_tabs)} $key: ${value.toString()}');
        } else {
          devLog('║${_indent(_tabs)} $key: [');
          _printList(value, tabs: _tabs);
          devLog('║${_indent(_tabs)} ]${isLast ? '' : ','}');
        }
      } else {
        ...
        if (msg.length + indent.length > linWidth) {
          ...
          devLog('║${_indent(_tabs)} $key: $msg${!isLast ? ',' : ''}');
        }
      }
    });

    devLog('║$initialIndent}${isListItem && !isLast ? ',' : ''}');
  }
